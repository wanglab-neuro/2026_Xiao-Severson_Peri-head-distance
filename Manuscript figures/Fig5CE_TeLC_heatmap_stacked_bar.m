...
% TeLC Dataset -- 1 way stats - wall distance for chemogenetic silencing experiment
...
settings.data_folder = 'E:\PrV_Wall_Recordings\Analysis\IntermediateData\TeLC';
dataTable = matfile(fullfile(settings.data_folder, 'tuningTable_PrV.mat')).dataTable;

% Load spike stats table
sst = matfile(fullfile(settings.wv_stats, 'PrV_spike_stats_table_telc.mat')).sst;
...
for neuron = size(dataTable,1):-1:1
    if included_units(neuron)
        try
            ...
            % if any(d2remove)
            %     d2remove_idx = find(d2remove);
            %     for dd = 1:numel(d2remove_idx)
            %         baseline_data{d2remove_idx(dd)} = repmat(baseline_data{d2remove_idx(dd)},2,1);
            %         wallpass_data{d2remove_idx(dd)} = repmat(wallpass_data{d2remove_idx(dd)},2,1);
            %     end
            % end
...
%% Panels C, D: Plot population heatmap 
% Settings
use_interp = 1;
...
% First sort by average slope
%     fr_mean_diff = gradient(fr_mean_norm,2);
%     slope_fr = mean(fr_mean_diff,2,'omitnan');
%     [~,slope_order] = sort(slope_fr,'descend');
%     fr_mean_norm = fr_mean_norm(slope_order,:);
% 
%     % Second, sort by number of non-NaN bins
%     num_fr = sum(~isnan(fr_mean_norm),2);
%     [~,num_order] = sort(num_fr,'descend');
%     fr_mean_norm = fr_mean_norm(num_order,:);
...
%% Panel E: Define proximity (monotonic) vs. map cells for population decoding and plot stacked bar chart
save_plots = 1;
...
% Panel E: Plot stacked bar chart
% Run statistical test (Z-test) to compare TeLC vs. Naive population proportions
% run top section first to load settings variable
clearvars -except settings

% Test hypothesis that proportion of map cells is lower in TeLC population
subpopulation_names = matfile(fullfile(settings.fig_dir, 'piechart_source_data_naive')).names;
naive_data = matfile(fullfile(settings.fig_dir, 'piechart_source_data_naive')).slices;
telc_data = matfile(fullfile(settings.fig_dir, 'piechart_source_data_telc')).slices;

% Get bootstrapped confidence intervals on proportions for each population
[Z, p_value, ci_naive, ci_telc] = deal(zeros(numel(subpopulation_names),1));
for p = 1:numel(subpopulation_names)

    % Estimate bootstrapped confidence intervals on naive map cell proportion
    d = false(sum(naive_data),1);
    d(1:naive_data(p)) = true;
    ci = bootci(1000, @(x) sum(x), d);
    ci_naive(p,1) = round(ci(2)-ci(1)/2,2) / sum(d);
    disp(['Naive ' subpopulation_names{p} ': ' num2str(round(100*naive_data(p)/sum(naive_data),2)) ' +/- ' num2str(ci_naive(p,1)) '% (95% CI)'])

    % Estimate bootstrapped confidence intervals on TeLC map cell proportion
    d = false(sum(telc_data),1);
    d(1:telc_data(p)) = true;
    ci = bootci(1000, @(x) sum(x), d);
    ci_telc(p,1) = round(ci(2)-ci(1)/2,2) / sum(d);
    disp(['TeLC ' subpopulation_names{p} ': ' num2str(round(100*telc_data(p)/sum(telc_data),2)) ' +/- ' num2str(ci_telc(p,1)) '% (95% CI)'])

    % Run Z-test on data from pie charts (written by Chat-GPT)
    n1 = sum(naive_data); % Total number of cells in group 1
    n2 = sum(telc_data);  % Total number of cells in group 2
    
    x1 = naive_data(p); % Number of map cells in group 1
    x2 = telc_data(p); % Number of map cells in group 2
    
    % Proportions
    p_tuned = x1 / n1; % Proportion in group 1
    p2 = x2 / n2; % Proportion in group 2
    
    % Pooled proportion
    p_pooled = (x1 + x2) / (n1 + n2);
    
    % Z-test statistic
    Z(p) = (p_tuned - p2) / sqrt(p_pooled * (1 - p_pooled) * (1/n1 + 1/n2));
    
    % One-tailed p-value (assuming p1 > p2 as the alternative hypothesis)
    ...
    % map or suppressed should decrease (one-tailed hypothesis: p2 > p1)
    if p==3 || p==4 
        p_value(p,1) = 1 - normcdf(Z(p));
        fprintf('P-value (one-tailed, p2 > p1): %.7f\n', p_value(p));

    % proximity (one-tailed hypothesis: p2 < p1)
    elseif p==1
        p_value(p,1) = normcdf(Z(p));
        fprintf('P-value (one-tailed, p2 < p1): %.7f\n', p_value(p));

    % untuned, ambiguous - not sure which direction
    elseif p==2 || p==5
        p_value(p,1) = 2 * (1 - normcdf(abs(Z(p)))); % two-tailed
        fprintf('P-value (two-tailed): %.7f\n', p_value(p));

    end
    
    % Decision
    if p_value(p,1) < 0.05 && (p==3 || p==4)
        disp(['The proportion of ' subpopulation_names{p} ' cells is significantly lower in group 2 (p < 0.05).']);
    elseif p_value(p,1) < 0.05 && (p==1 || p==2)
        disp(['The proportion of ' subpopulation_names{p} ' cells is significantly greater in group 2 (p < 0.05).']);
    elseif p_value(p,1) < 0.05 && (p==5 || p==2)
        disp(['The proportion of ' subpopulation_names{p} ' cells is significantly different between group 1 and group 2 (p >= 0.05).']);
    else
        disp(['The proportion of ' subpopulation_names{p} ' cells is not significantly different between group 1 and group 2 (p >= 0.05).']);
    end
    disp(newline)
    
    % map cells lower in group 2 (telc) p = 7.1e-4
end

% Export source data table
mean_naive = 100 * shiftdim(naive_data / sum(naive_data));
mean_telc = 100 * shiftdim(telc_data / sum(telc_data));
results = table(shiftdim(subpopulation_names), mean_naive, ci_naive, mean_telc, ci_telc, Z, p_value, ...
    'VariableNames',{'population','naive (%)', 'ci_naive (%)', 'telc (%)', 'ci_telc (%)', 'Z', 'pvalue'});
writetable(results, fullfile(settings.fig_dir, 'piechart_telc_naive_source_data_analysis.csv'))

% Plot stacked bar chart to compare 
save_plots = 1;

% Flip colors, population names so that untuned is on the bottom
newColors = flip([1 0.6 0.6; 1 0.8 0.6; 1 1 0.6; 0.6 0.6 1; 0.6 0.6 0.6]);
...
x = [1; 2];
y = [flip(naive_data) / sum(naive_data); flip(telc_data) / sum(telc_data)];
b = bar(x, y, 'stacked', 'FaceColor','flat', 'EdgeColor','k', 'LineWidth',0.5);
lgd = legend(flip(subpopulation_names), 'Box','off', 'Location','eastoutside', 'FontSize',8);
for i = 1:numel(b)
    b(i).CData = newColors(i,:);
    disp([subpopulation_names{i} ' Wild-type : ' num2str(100*y(1,i)) '%, TeLC: ' num2str(100*y(2,i)) '%'])
end
set(gca, 'Box','off', 'TickDir','out', 'XTickLabels',{'Control','TeLC'}, 'FontSize',8, 'FontName','arial')
axis square
ylabel('Frac. Units')

if save_plots
    filename = fullfile(settings.fig_dir, ['stacked_bar_naive_vs_telc_prox_map_proportion']);
    savefig(gcf, filename);
    saveas(gcf, filename, 'png');
    saveas(gcf, filename, 'pdf');
    pause(0.01)
