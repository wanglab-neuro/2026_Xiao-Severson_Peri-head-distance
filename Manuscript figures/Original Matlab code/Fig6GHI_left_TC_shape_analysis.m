...
% Load sst
wv_stats = fullfile(dl, 'Kyle\Data\WallPassing\PrV_Wall_Recordings\WaveformStats_naive\PrV_spike_stats_table_naive.mat');
sst = matfile(wv_stats).sst;

wv_stats_telc = fullfile(dl, 'Kyle\Data\WallPassing\PrV_Wall_Recordings\WaveformStats_telc\PrV_spike_stats_table_telc.mat');
sst_telc = matfile(wv_stats_telc).sst;

% Load tuning curves from naive and telc datasets
dataTable = matfile(fullfile(base_dir, 'Naive', 'tuningTable_PrV_stats.mat')).dataTable;
dataTable_telc = matfile(fullfile(base_dir, 'TeLC', 'tuningTable_PrV_stats.mat')).dataTable;
...
%% Panel G (left): Plot FWHM CDF for Naive vs. TeLC

x_wt = dataTable.FWHM;
x_telc = dataTable_telc.FWHM;

mask_wt = get_wall_tuned_mask(dataTable, sst, x_wt);
mask_telc = get_wall_tuned_mask(dataTable_telc, sst_telc, x_telc);

plot_group_cdf(x_wt(mask_wt), x_telc(mask_telc), ...
    'Tuning curve width (mm)', ...
    90211, ...
    'CDF_full_width_half_max_WT_vs_TeLC', ...
    fig_dir, ...
    'XLim',[0 18]);

%% Panel H (left): Plot modulation depth CDF for Naive vs. TeLC (on log scale)
...
fig = plot_group_cdf_log( ...
    x_wt(mask_wt), ...
    x_telc(mask_telc), ...
    'Modulation depth (sp/s)', ...
    90212, ...
    'CDF_modulation_depth_WT_vs_TeLC', ...
    fig_dir, ...
    'XLim',[0 100]);

%% Panel I: Plot signed monotonicity index histogram for Naive vs. TeLC side-by-side
...
plot_group_cdf(x_wt(mask_wt), x_telc(mask_telc), ...
    ...
    90210, ...
    'CDF_signed_monotonicity_index_WT_vs_TeLC', ...
    fig_dir, ...
    'XLim',[-1 1]);
...
function fig = plot_group_cdf_log(x_wt, x_telc, xlab, figNum, filename, fig_dir, varargin)
